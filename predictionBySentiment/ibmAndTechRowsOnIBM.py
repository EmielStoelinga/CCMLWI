...
and all technology headlines in which the term 'ibm' does not reside.
...
def filter_out_ibm(data):
	return data[~data['normalized_headline'].str.contains("ibm")].values
	...
	for i in tqdm(range(0, len(microsoft_data))):
		...
		if microsoft_date == tech_data[tech_index, 0]:
			...
			if not microsoft_date == tech_data[tech_index+1, 0]:
				print "Day of IBM headlines missing."
				tech_index += 1
			else:
				tech_index += 1
				avg_sents[i] = tech_data[tech_index, 1]
...
def main():
	microsoft_data = import_data("./../data/all_data/combined_IBM_IBM_tech_news.csv").values
	tech_data = import_data("./../data/all_data/combined_IBM_tech_news.csv")
	print("Data reading done.")
	filtered_tech_data = filter_out_ibm(tech_data)
	print("Filtering tech data done.")

	microsoft_sentiment_included = add_sentiment(microsoft_data)
	print("Adding sentiments to IBM done.")
