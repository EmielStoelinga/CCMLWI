...
Google stock with headlines on technology in which the term 'google' resides
and all technology headlines in which the term 'google' does not reside.
...
def filter_out_google(data):
	return data[~data['normalized_headline'].str.contains("google")].values
	...
	for i in tqdm(range(0, len(microsoft_data))):
		...
		if microsoft_date == tech_data[tech_index, 0]:
			...
			if not microsoft_date == tech_data[tech_index+1, 0]:
				print "Day of Google headlines missing."
				tech_index += 1
			else:
				tech_index += 1
				avg_sents[i] = tech_data[tech_index, 1]
...
def main():
	microsoft_data = import_data("./../data/all_data/combined_GOOG_google_tech_news.csv").values
	tech_data = import_data("./../data/all_data/combined_GOOG_tech_news.csv")
	print("Data reading done.")
	filtered_tech_data = filter_out_google(tech_data)
	print("Filtering tech data done.")

	microsoft_sentiment_included = add_sentiment(microsoft_data)
	print("Adding sentiments to google done.")
